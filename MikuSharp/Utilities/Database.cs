...
public class Database
{
	...
	public static async Task<List<QueueEntry>> GetQueueAsync(DiscordGuild g)
	{
		...
		while (await reader2.ReadAsync())
			queue.Add(new(await MikuBot.LavalinkSessions.First().Value.DecodeTrackAsync(Convert.ToString(reader2["track"])), Convert.ToUInt64(reader2["user_id"]), DateTimeOffset.Parse(reader2["added_at"].ToString()), Convert.ToInt32(reader2["position"])));
