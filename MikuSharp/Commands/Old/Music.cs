...
public class Music : ApplicationCommandsModule
{
    ...
    public class Playback : ApplicationCommandsModule
    {
        ...
        )
        {
            ...
            if (curq.Count != 0 && g.MusicInstance.Playstate == Playstate.NotPlaying)
            {
                ...
                List<DiscordButtonComponent> buttons = [new(ButtonStyle.Success, "restore", "Restore old queue"), new(ButtonStyle.Danger, "clear", "Clear old queue")];
                var msg = await ctx.EditResponseAsync(new DiscordWebhookBuilder()
                    .AddEmbed(new DiscordEmbedBuilder().WithTitle("Recover Queue").WithDescription("The last time the bot disconnected the queue wasnt cleared, do you want to restore and play that old one?").Build()).AddComponents(buttons));
            ...
            {
                emb.AddField(new(q.Tracks.First().Info.Title +
                                 "[" +
                                 (q.Tracks.First().Info.Length.Hours != 0
                                     ? q.Tracks.First().Info.Length.ToString(@"hh\:mm\:ss")
                                     : q.Tracks.First().Info.Length.ToString(@"mm\:ss")) +
                                 "]", $"by {q.Tracks.First().Info.Author}\n" + $"Requested by {ctx.Member.Mention}"));
                if (q.Tracks.Count != 1)
            ...
            else
            {
                if (q.PlaylistInfo.SelectedTrack == -1 || q.PlaylistInfo.Name == null)
                    ...
                    emb.AddField(new(q.Tracks[q.PlaylistInfo.SelectedTrack].Info.Title +
                                     "[" +
                                     (q.Tracks[q.PlaylistInfo.SelectedTrack].Info.Length.Hours != 0
                                         ? q.Tracks[q.PlaylistInfo.SelectedTrack].Info.Length.ToString(@"hh\:mm\:ss")
                                         : q.Tracks[q.PlaylistInfo.SelectedTrack].Info.Length.ToString(@"mm\:ss")) +
                                     "]",
    ...
    public class Queue : ApplicationCommandsModule
    {
        ...
        public static async Task ShowQueueAsync(InteractionContext ctx)
        {
            ...
            try
            {
                ...
                if (g.MusicInstance.RepeatMode == RepeatMode.All)
                {
                    ...
                    foreach (var track in queue)
                    {
                        if (songsPerPage == 0 && currentPage == 1)
                        {
                            ...
                            emb.AddField(new(
                                $"**{songAmount}.{g.MusicInstance.CurrentSong.Track.Info.Title.Replace("*", "").Replace("|", "")}** by {g.MusicInstance.CurrentSong.Track.Info.Author.Replace("*", "").Replace("|", "")} [{time1}/{time2}]",
                ...
                if (currentPage == 2 && songsPerPage == 0)
                {
                    await ctx.EditResponseAsync(new DiscordWebhookBuilder().AddEmbed(pages.First().Embed));
                    return;
                }
    ...
    public class PlaybackOptions : ApplicationCommandsModule
    {
        ...
        public static async Task ShuffleAsync(InteractionContext ctx)
        {
            ...
            g.MusicInstance.UsedChannel = ctx.Channel;
            g.MusicInstance.ShuffleMode = g.MusicInstance.ShuffleMode == ShuffleMode.Off
                ? ShuffleMode.On
                : ShuffleMode.Off;
